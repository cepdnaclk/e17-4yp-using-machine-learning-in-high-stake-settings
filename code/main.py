...
log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Initializing...')

data_file_path = config.PROCESSED_DATA_PATH
load_processed_data = config.LOAD_PROCESSED_DATA_FLAG


log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Creating classifier parameters.')
...
log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Creating directories if they do not exist.')
...
log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Loading data from csv.')

if load_processed_data:
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Loading preprocessed data.')
    ...
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Laoding data and preprocessing.')
    print("Start data pre processing")
    data = dp.load_data_to_df(config.DATA_SOURCE)

    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Casting datetime datatype.')
    data = dp.set_data_types_to_datetime(data, config.DATE_COLS)

    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Imputing data.')
    ...
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Selecting only the required features from df.')
    ...
    # Adding new features
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Adding new features.')
    ...
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, f'After adding new Features, shape = {data.shape}, columns = {data.columns}')

    # export labelled data to csv
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Exporting labelled data to csv.')
    dp.export_data_frame(data=data, path=data_file_path)
    print(f"Saved data as csv at {data_file_path}")



log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Encoding data.')
...
log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Starting loop for each model.')
for model_item in models:

    print(f"Classifier -> {model_item.get('model_name')}")
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, f"Classifier -> {model_item.get('model_name')}")
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Start pipeline for model.')

    trained_model, eval_metrics = fe.run_pipeline(
        data=data_1, model=model_item
    )

    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'End pipeline for model.')

    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Plotting precision curves and saving hyperparameters.')

    print("k_fixed_precisions = ", eval_metrics.get("k_fixed_precision"))
    log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, f'k_fixed_precisions = {eval_metrics.get("k_fixed_precision")}')
...
log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Save hyperparameter table for all models.')
...
log_intermediate_output_to_file(config.INFO_DEST, config.PROGRAM_LOG_FILE, 'Plot precision curves for all models.')
